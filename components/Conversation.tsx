...
export function Conversation({ view, onViewChange, headerRight }: ConversationProps) {
      ...
      void Promise.all([fetchProgress(authUserId), fetchTodayConversation(authUserId)])
        .then(([snapshot, conversation]) => {
          ...
          if (conversation.messages.length > 0) {
            ...
            // Allow mic continue without forcing a brand-new opening greeting.
  ...
  // Persist today's transcript so refresh can resume the thread.
  useEffect(() => {
  ...
  // Credit practice time to Firestore while a session is running.
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      void postProgress({ userId, addSeconds: delta })
        ...
        .catch((error: unknown) => {
          console.error(error);
          // Roll back credit marker so we retry next interval.
    ...
    async (audioBlob: Blob) => {
      ...
      try {
        ...
        if (!transcribeRes.ok || !transcribeJson.text) {
          // In continuous mode, quietly listen again instead of hard-stopping the flow.
          if (
            autoContinueRef.current &&
            settingsRef.current.continuousConversation &&
            transcribeRes.status === 422
          ) {
            setStatus("ready");
  ...
  const startListening = useCallback(async () => {
    ...
    // Resume an active timed session if needed.
    if (startedAt == null) {
      setStartedAt(Date.now());
      setSessionStarted(true);
      sessionCreditedRef.current = 0;
    ...
    try {
      ...
      const stream = await Promise.race([
        ...
        new Promise<MediaStream>((_, reject) => {
          window.setTimeout(() => {
            reject(
              Object.assign(new Error("Microphone request timed out"), {
                name: "TimeoutError",
              }),
            );
          }, 12_000);
      ...
      if (name === "NotAllowedError" || name === "PermissionDeniedError") {
        fail("Microphone permission denied. Allow the mic, then try again.");
        return;
      }
      if (name === "NotFoundError" || name === "DevicesNotFoundError") {
        fail("No microphone found. Plug one in and try again.");
        return;
      }
      if (name === "TimeoutError") {
        fail("Microphone permission timed out. Allow the mic in your browser, then try again.");
        return;
      }
  ...
  const handleMicClick = () => {
    ...
    if (status === "listening" || status === "connecting") {
      // Stop recording without sending, and stay paused.
  ...
  const handleClearChat = () => {
    ...
    if (userIdRef.current) {
      void clearTodayConversationClient(userIdRef.current).catch(console.error);
    }
  };
  ...
  const handleSettingsChange = (next: PracticeSettings) => {
    ...
    setSettings(next);
    if (goalChanged) {
      void saveDailyGoal(next.dailyGoalMinutes);
    }
  };
  ...
  const saveDailyGoal = (dailyGoalMinutes: number) => {
    ...
    void postProgress({
      ...
      .catch((error: unknown) => {
        console.error(error);
        setProgressError(
          error instanceof Error ? error.message : "Couldn't save daily goal.",
        );
      });
  };

  const micDisabled =
    (!sessionStarted && !hasThread) ||
    status === "transcribing" ||
    status === "thinking" ||
    status === "speaking";
    ...
    <div className="mx-auto flex h-dvh w-full max-w-2xl flex-col overflow-hidden px-5 pb-5 pt-6 sm:px-8 sm:pb-6 sm:pt-8">
      <header className="relative mb-4 shrink-0 text-center">
        ...
        <div className="mt-3 flex flex-wrap items-center justify-center gap-3">
          <p className="inline-flex items-center rounded-md border border-[var(--level-border)] px-2.5 py-1 text-xs font-medium uppercase tracking-[0.16em] text-[var(--muted)]">
            {settings.level}
          </p>
        ...
        {progressError ? (
          <p className="mt-2 text-xs text-[var(--danger)]">{progressError}</p>
        ) : null}
        <p className="mt-2 text-sm text-[var(--muted)]">
          Chatting with {partner.name} in {partner.city}
        </p>
      </header>

      <section
        className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-[1.5rem] border border-[var(--panel-border)] bg-[var(--panel)]/80 shadow-[0_20px_60px_rgba(28,42,36,0.06)] backdrop-blur-sm"
        aria-label="Conversation transcript"
      >
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain px-5 py-5 sm:px-7">
          {messages.length === 0 ? (
            <div className="flex h-full min-h-[180px] flex-col items-center justify-center gap-5 text-center">
              <p className="max-w-sm text-[1.05rem] leading-relaxed text-[var(--muted)]">
                Start a session and {partner.name} will open in Spanish. Then keep talking
                naturally — after a short silence your turn sends automatically. Use Translate
                anytime you need the English meaning.
              </p>
              <button
                type="button"
                ...
                className="rounded-full bg-[var(--accent)] px-6 py-3 text-sm font-medium text-white transition hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {status === "thinking" || status === "speaking"
                  ? `${partner.name} is joining…`
                  : "Start conversation"}
              </button>
            </div>
          ) : (
            messages.map((message) => (
              <Message
                ...
                role={message.role}
                content={message.content}
                speakerName={message.speakerName ?? partner.name}
              />
            ))
          )}
      ...
      <div className="mt-4 flex shrink-0 flex-col items-center gap-4">
        ...
        <div className="flex items-center gap-4 text-sm text-[var(--muted)]">
          <button
            ...
            disabled={!canReplay || status === "listening" || micDisabled}
            className={[
              "underline-offset-4 hover:text-[var(--foreground)] hover:underline disabled:cursor-not-allowed disabled:no-underline disabled:opacity-40",
              needsTapToPlay ? "font-semibold text-[var(--partner)]" : "",
            ].join(" ")}
          >
          ...
          <button
            ...
            disabled={!startedAt || status === "listening" || status === "connecting"}
            className="underline-offset-4 hover:text-[var(--foreground)] hover:underline disabled:cursor-not-allowed disabled:no-underline disabled:opacity-40"
          >
            Pause
          </button>
          <span aria-hidden>·</span>
          <button
            ...
            disabled={!hasThread || status === "listening" || status === "connecting"}
            className="underline-offset-4 hover:text-[var(--foreground)] hover:underline disabled:cursor-not-allowed disabled:no-underline disabled:opacity-40"
          >
            Clear chat
          </button>
