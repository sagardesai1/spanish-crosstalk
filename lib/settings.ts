...
const PARTNERS: Record<SpanishRegion, Record<VoiceGender, PartnerPersona>> = {
  spain: {
    male: {
      ...
      hometownNote:
        "He grew up near the Mercado Central, knows good tortilla spots, quiet plazas, and easy day trips to the coast.",
    },
    female: {
      name: "Lucía",
      city: "Valencia",
      ...
      hometownNote:
        "She knows sunny terraces near the Turia gardens, simple lunch spots, and calm evening paseos by the sea.",
    },
  },
  latin_america: {
    male: {
...
export function levelGuidance(level: LearnerLevel): string {
  switch (level) {
    case "A1":
      return "Use the simplest everyday words. Keep sentences very short. Prefer present tense. Avoid conjugations beyond basics.";
    case "A2":
      return "Still simple and clear, but you may use a little past (pretérito) and future plans with voy a. Keep sentences short.";
    case "B1":
      return "Natural everyday speech with a bit more detail. Short-to-medium sentences are fine. Occasional connectors like porque, entonces, también.";
    case "B2":
      return "Comfortable conversational Spanish with richer vocabulary, while staying clear and friendly. Avoid rare idioms.";
    case "C1":
      return "Fluent natural conversation with nuance, still warm and patient. Prefer clear phrasing over showy complexity.";
    case "C2":
      return "Fully natural native-like conversation. Stay engaging and human, not academic.";
  }
}

export function dialectGuidance(region: SpanishRegion): string {
  if (region === "spain") {
    return "Speak peninsular Spanish from Spain. You may use vosotros when it fits naturally.";
  }
  return "Speak Latin American Spanish (Mexico-friendly). Prefer ustedes over vosotros. Avoid Spain-only slang.";
}
